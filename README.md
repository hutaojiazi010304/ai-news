...
此外，伯乐精选每周会被整理成一篇精读版推文预览页（`weixin-deep/`），用于企业内部服务号的内部分发。千问 API key 有 IP 白名单限制、只能在本地网络调用，因此推文改为每周本地手动运行 `scripts/generate_weixin_article_deep.py` 生成并提交（原 `weixin-daily.yml` 工作流已移除）；配置与取稿流程见 [docs/WEIXIN_DAILY.md](docs/WEIXIN_DAILY.md)。该步骤只消费现有管线产物，不改动抓取流程；不配置千问 key 也能降级运行。
