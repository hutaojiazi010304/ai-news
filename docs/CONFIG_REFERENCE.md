...
- **峰值分 ≥ 0.72**(`BRIEF_SCORE_GATE`),或
- **来源数 ≥ 2 且峰值分 ≥ 0.65**(`BRIEF_MULTI_SOURCE_SCORE_GATE`)。多源印证也要有
  质量下限:同一个论坛帖/仓库被多个镜像通道转载时,管道里会刷出虚假的"多来源",
  只数来源数会把低价值内容抬进精选。这里的"来源数"(`source_count`)按去重后的
  canonical URL 计(`distinct_story_source_count`):同一 URL 的重复抓取与镜像通道
  只算一个来源;`sources`/`items` 列表和 `duplicate_count` 仍保留全部管道条目供追溯。
  峰值分(`peak_score`)持久化在
...
热度项按故事的**独立来源数**(去重后的 canonical URL 数)计算:
`min(1, (独立来源数 - 1) / 4)`,同一 URL 的镜像/重抓不叠加热度。
